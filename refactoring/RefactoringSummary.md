...
* Mengimplementasikan refactor akhir untuk `Switch Statements` menggunakan enum `MenuOption`.
* Memisahkan `ExpenseTracker` menjadi entry point ringan dan `ExpenseTrackerApp` sebagai application runner.
* Mengevaluasi leftover code smell dan mempertahankan cleanup tetap minimal untuk menghindari overengineering.
...
* **Switch Statements:** Struktur kondisional besar yang mengontrol banyak action dalam satu titik.
* **God Class / Large Class:** Kelas yang memiliki terlalu banyak responsibility atau ukuran terlalu besar.

### Alasan Implementasi Sebelumnya Termasuk Code Smell

* `ExpenseTracker` sebelumnya menggunakan `switch(choice)` untuk seluruh aksi menu.
* `ExpenseTracker` juga menangani application flow, menu handling, dan state aplikasi secara bersamaan.

### Perubahan yang Dilakukan

* Mengganti `switch` dengan enum `MenuOption`.
* Memindahkan orchestration aplikasi ke `ExpenseTrackerApp`.
* Menjadikan `ExpenseTracker` hanya sebagai entry point.

### Alasan Refactor Meningkatkan Kualitas

* **Maintainability:** Penambahan menu baru lebih modular.
* **Readability:** Flow aplikasi lebih jelas.
* **Modularity:** Responsibility dipisahkan antara entry point dan application runner.
* **Scalability:** Struktur lebih mudah dikembangkan tanpa memperbesar conditional block.

### File yang Diubah

* `src/ExpenseTracker.java`
* `src/ExpenseTrackerApp.java`
...
* Logic menu handling dipindahkan dari `switch statement` ke enum `MenuOption`.
* Responsibility orchestration aplikasi dipindahkan dari `ExpenseTracker` ke `ExpenseTrackerApp`.

### Teknik Refactor yang Digunakan

* `Replace Conditional with Polymorphism using Enum-based Command Pattern`
* `Extract Class`
* `Move Method`
* `Encapsulate Field`
...
* `ExpenseTrackerApp` masih memiliki beberapa method CLI dan state aplikasi, tetapi masih dianggap acceptable untuk aplikasi console sederhana.
* Tidak dilakukan redesign tambahan untuk menghindari overengineering.
* Tidak ada `switch statement` besar yang tersisa pada project.
* Tidak dilakukan redesign tambahan untuk menghindari overengineering dan menjaga struktur aplikasi tetap sederhana.
* Beberapa code smell ringan yang masih tersisa tetap dipertahankan karena masih dianggap acceptable untuk skala aplikasi console ini dan belum memberikan dampak signifikan terhadap maintainability.
